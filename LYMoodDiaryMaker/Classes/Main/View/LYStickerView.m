...
#import "LYStickerView.h"

@interface LYStickerView ()<UIGestureRecognizerDelegate>
...
- (instancetype)initWithContentView:(UIView *)contentView {
    ...
    if (self) {
        ...
        [self showScaleCtrl:YES];

        self.ctrlType = LYStickerViewCtrlTypeGesture;//默认为手势模式
...
- (void)setCtrlType:(LYStickerViewCtrlType)ctrlType {
    _ctrlType = ctrlType;
    switch (_ctrlType) {
        case LYStickerViewCtrlTypeGesture:
...
- (void)setOriginalPoint:(CGPoint)originalPoint {
    //    if (self.ctrlType == LYStickerViewCtrlTypeGesture) {
...
- (CGFloat)initialAngle {
    if (self.ctrlType == LYStickerViewCtrlTypeOne) {
    ...
    if (self.ctrlType == LYStickerViewCtrlTypeTwo) {
...
- (void)pinch:(UIPinchGestureRecognizer *)gesture {
    ...
    if (self.scaleMode == LYStickerViewScaleModeBounds) {

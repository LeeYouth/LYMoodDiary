...
#import "LYFuncTestViewController.h"
#import <LYCommonKit/LYCommonKit.h>
...
@interface LYFuncTestViewController ()<WKUIDelegate,WKNavigationDelegate,UIScrollViewDelegate>
...
@implementation LYFuncTestViewController
...
//    [webView evaluateJavaScript:fontFamilyStr completionHandler:nil];
    ...
    if ([self.dk_manager.themeVersion isEqualToString:DKThemeVersionNight]) {
        ...
        [webView evaluateJavaScript:@"document.body.style.webkitTextFillColor=\"#000000\"" completionHandler:nil];
    }
...
- (void)webView:(WKWebView *)webView didFailProvisionalNavigation:( WKNavigation *)navigation withError:(NSError *)error{
    ...
    if (self.webView.canGoBack == 1)
    {
        NSLog(@"self.webView.canGoBack == 1");
//        [self hideEmptyView];
    }else
    {
//        [self showEmptyDataViewWithTpye:MTShowEmptyDataTpyeNoNetwork];
        NSLog(@"self.webView.canGoBack == 0");
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (offset >= NAVBAR_HEIGHT) {
        self.navBarView.navBarTitle = self.titleStr.length?self.titleStr:self.title;
...
- (WKWebView *)webView {
    if (!_webView) {
        ...
        _webView.scrollView.dk_backgroundColorPicker = bgColor;
...
- (UIProgressView *)progressView{
    if (!_progressView) {
        ...
        _progressView.dk_progressTintColorPicker = themeColor;

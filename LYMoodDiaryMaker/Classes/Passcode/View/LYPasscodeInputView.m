...
@implementation LYPasscodeInputView
...
- (void)_initialize
{
    ...
    _enabled = YES;
    _passcodeStyle = LYPasscodeInputViewNumericPasscodeStyle;
...
- (UIControl *)passcodeField
{
    if (nil == _passcodeField) {
        ...
        switch (_passcodeStyle) {
            case LYPasscodeInputViewNumericPasscodeStyle:
            {
                if (_maximumLength == 0) {
                ...
                LYPasscodeField *passcodeField = [[LYPasscodeField alloc] init];
            ...
            case LYPasscodeInputViewNormalPasscodeStyle:
            {
                if (_maximumLength == 0) {
...
- (void)setMaximumLength:(NSUInteger)maximumLength
{
    ...
    if ([_passcodeField isKindOfClass:[LYPasscodeField class]]) {
        [(LYPasscodeField *)_passcodeField setMaximumLength:maximumLength];
    }
}
...
- (NSString *)passcode
{
    switch (self.passcodeStyle) {
        case LYPasscodeInputViewNumericPasscodeStyle:
            return [(LYPasscodeField *)self.passcodeField passcode];
        case LYPasscodeInputViewNormalPasscodeStyle:
...
- (void)setPasscode:(NSString *)passcode
{
    switch (self.passcodeStyle) {
        case LYPasscodeInputViewNumericPasscodeStyle:
            [(LYPasscodeField *)self.passcodeField setPasscode:passcode];
            break;
        case LYPasscodeInputViewNormalPasscodeStyle:
...
{
    if (![self.passcodeField isKindOfClass:[LYPasscodeField class]]) {
        return;
    }
    
    LYPasscodeField *passcodeField = (LYPasscodeField *)self.passcodeField;
...
#pragma mark - LYPasscodeFieldDelegate
...
{
    LYPasscodeInputView *view = [[[self class] alloc] initWithFrame:self.bounds];
